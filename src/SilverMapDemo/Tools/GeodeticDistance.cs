...
namespace ESRI.ArcGIS.Samples
{
	public class GeodeticDistance : DependencyObject
	{
		...
		private void map_MouseMove(object sender, MouseEventArgs e)
		{
			...
			dist = Geodesic.GetSphericalDistance(origin, p);
			textSymb.Text = string.Format("{0} km", RoundToSignificantDigit(dist, map.Resolution));

			greatCircleLine.Geometry = Geodesic.GetGeodesicLine(origin, p);
			radiusLine.Geometry = Geodesic.GetRadius(origin, dist);
			radiusFill.Geometry = Geodesic.GetRadiusAsPolygon(origin, dist);
