...
namespace ESRI.ArcGIS.Samples
{
	...
	public class HeatMapLayer : DynamicLayer
	{
		...
		public bool GenerateRandomData
		{
			...
			set
			{
				...
				if (value)
				{
					...
					{
						double x = rand.NextDouble() * 60 - 130;
						double y = rand.NextDouble() * 40 + 20;
						HeatMapPoints.Add(new ESRI.ArcGIS.Client.Geometry.MapPoint(x, y));
					}
				}
			}
		}
		...
		protected  void GetSource(Envelope extent, int width, int height, DynamicLayer.OnImageComplete onComplete)
		{
			if (!IsInitialized)
			{
